...
export async function POST(req: NextRequest) {
  ...
  const memDoc = getStoredDocument(documentId);
  if (memDoc) {
  ...
  // 3. Generate Questions (Claude Sonnet RAG or In-Memory Synthesis)
  ...
  // Grounded fallback if LLM offline or key not provided
  if (generatedList.length === 0) {
    const fallbackPool =
      SAMPLE_QUESTIONS_DATABASE[documentId] ||
      SAMPLE_QUESTIONS_DATABASE["doc_plfs_2024"] ||
      Object.values(SAMPLE_QUESTIONS_DATABASE).flat();

    const matching = fallbackPool.filter((q) =>
      competencyFracCodes.includes(q.competencyFracCode)
    );

    const poolToUse = matching.length > 0 ? matching : fallbackPool;
    generatedList = poolToUse.slice(0, questionCount);
    ...
    if (generatedList.length < questionCount && chunks.length > 0) {
      for (let i = generatedList.length; i < questionCount && i < chunks.length; i++) {
        ...
        const frac = competencyFracCodes[i % competencyFracCodes.length] || "FN-STAT-014";
        generatedList.push({
