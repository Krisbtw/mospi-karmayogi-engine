        ...
        <CardContent className="pt-2">
          <div className="relative h-[300px] w-full">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={chartData} outerRadius="62%" margin={{ top: 24, right: 20, bottom: 24, left: 20 }}>
                <PolarGrid stroke="#dce6ef" strokeDasharray="2 2" />
                <PolarAngleAxis
                  dataKey="label"
                  tick={({ payload, x, y, textAnchor }) => {
                    ...
                    return <text x={x} y={y} textAnchor={textAnchor} fill="#597293" fontSize={10} fontWeight={500} fontFamily="var(--font-ui), sans-serif">{lines.map((line, index) => <tspan key={index} x={x} dy={index === 0 ? -(lines.length - 1) * 6 : 12}>{line}</tspan>)}</text>;
                  }}
                />
                <PolarRadiusAxis
                  angle={90}
                ...
                <Tooltip
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="rounded-md border border-border bg-surface p-3 font-mono text-xs shadow-lg">
                          <p className="font-semibold text-fg">{item.label}</p>
                          <p className="text-fg-muted">{item.fracCode}</p>
                          <div className="mt-2 flex flex-col gap-1">
                            ...
                            {item.gap > 0 ? (
                              <div className="flex justify-between gap-4 border-t border-border pt-1 text-rose-700">
                                <span>Deficit</span>
                                <span className="font-semibold">−{item.gap}</span>
                              </div>
                            ) : (
                              <div className="border-t border-border pt-1 text-emerald-700">Benchmark met</div>
                            )}
                          </div>
                        </div>
                      );
